...
public class adminsignup extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final String[] registrationno = new String[1];
        ...
        signup_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                registrationno[0] =aadhar_txtedit.getText().toString().trim();
                phonenumber[0] =phone_textedit.getText().toString().trim();
                password[0] =pass_txtedit.getText().toString().trim();

                if (!fullname[0].equals("") && !phonenumber[0].equals("") && !password[0].equals("") && !email[0].equals("") && !registrationno[0].equals("")){

                    if (phonenumber[0].length() != 10){
                        Toast.makeText(adminsignup.this,"phone number is not correct",Toast.LENGTH_SHORT).show();
                    }
                    else if(password[0].length() < 6){
                        Toast.makeText(adminsignup.this,"password length must be greater than 5",Toast.LENGTH_SHORT).show();
                    }
                    else{
                            ...
                            admindetail.addListenerForSingleValueEvent(new ValueEventListener() {
                                ...
                                public void onDataChange(@NonNull @org.jetbrains.annotations.NotNull DataSnapshot snapshot) {
                                    if (snapshot.child(admin_emailcheck).exists()) {
                                        Toast.makeText(getApplicationContext(), "Registration Number Already registered", Toast.LENGTH_SHORT).show();
                                    } else {
                                                ...
                                                if (task.isSuccessful()) {
                                                    Objects.requireNonNull(fauth.getCurrentUser()).sendEmailVerification().addOnCompleteListener(new OnCompleteListener<Void>() {
                                                        ...
                                                        public void onComplete(@NonNull @NotNull Task<Void> task) {
                                                            if(task.isSuccessful())
                                                            {
                                                                adminsignupdataholder obj = new adminsignupdataholder(fullname[0], phonenumber[0], email[0], registrationno[0], password[0]);
